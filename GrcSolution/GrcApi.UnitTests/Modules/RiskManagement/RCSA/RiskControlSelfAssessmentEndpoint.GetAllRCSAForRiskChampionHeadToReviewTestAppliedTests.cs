...
{
    internal class RiskControlSelfAssessmentEndpointGetAllRCSAForRiskChampionHeadToReviewTestAppliedTests
    {
        [Test]
        public async Task CallMethod_ReturnsListOfRCSAForRiskChampionToReviewTestAppliedTests()
        {
            // Arrange
            ...
            {
                RiskControlSelfAssessmentUnit.Create(units[0].Id, documentRSCAProcess: Create(Stage.RiskChampionHeadReviewTestApplied)),
                RiskControlSelfAssessmentUnit.Create(units[1].Id, documentRSCAProcess: Create(Stage.RiskChampionReviewTestApplied)),
                RiskControlSelfAssessmentUnit.Create(units[2].Id, documentRSCAProcess: Create(Stage.RiskChampionHeadReviewTestApplied))
            };
            ...
            PagedResult<GetAllRCSAForRiskChampionDto> expectedRCSAUnitResult = rcsaUnitRepo
                .GetAllPaged(0, 0, t => t
                .Where(r => r.DocumentRSCAProcess.RCSAStage == Stage.RiskChampionHeadReviewTestApplied)
        ...
        //    //Act
        //    var response = await RiskControlSelfAssessmentEndpointGetAllRCSAForRiskChampionHeadReviewTestApplied.HandleAsync(rcsaUnitRepo);

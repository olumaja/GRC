...
namespace Arm.GrcApi.Modules.InternalControl
{
    ...
    * Get all control dashboard for internal control for action owner
    */
    public class GetActionownerInternalControlDashboardEndpoint
    {
        /// <summary>
        /// Get all control dashboard for internal control dashboard action owner
        /// </summary>
        /// <param name="repo"></param>
        /// <param name="currentUserService"></param>
        /// <returns></returns>
        public static async Task<IResult> HandleAsync(
            int pageSize, int pageNumber, string? status, DateTime? startDate, DateTime? endDate, IRepository<InternalControlDashboard> repo, ICurrentUserService currentUserService, IHttpContextAccessor httpContext
        )
        {
            ...
            try
            {
                ...
                var myTasks = repo.GetContextByConditon(x => x.ActionOwnerEmail.ToLower() == currentUserService.CurrentUserEmail.ToLower())
                                   .OrderByDescending(r => r.CreatedOnUtc);
                if (startDate != null || endDate != null)
                {
                    myTasks = myTasks.Where(d => d.CreatedOnUtc.Date >= startDate.Value.Date && d.CreatedOnUtc.Date <= endDate.Value.Date)
                                  .OrderByDescending(r => r.CreatedOnUtc);
                }
                    ...
                    var statusRecommended = new Dictionary<string, InternalControlDashboardStatus>
                {
                    ...
                    myTasks = myTasks.Where(d => d.Status == statusRecommended[getstatus])
                                .OrderByDescending(r => r.CreatedOnUtc);
                }
                ...
                var paginatedTasks = Pagination<InternalControlDashboard>.Create(myTasks, pageNumber, pageSize);

...
{
    public class RiskControlSelfAssessmentEndpointGetAllRCSAForRiskChampionUnitHeadToInitiateRCSA
    {
        public static async Task<IResult> HandleAsync(IRepository<RiskControlSelfAssessmentUnit> repository, IRepository<Unit> unitRepo, ICurrentUserService currentUserService)
        {
			try
			{
                ...
                rcsaList = repository
                    .GetContextByConditon(u => u.Unit.Name == unit.Name).Where(r => r.DocumentRSCAProcess.RCSAStage == DocumentRSCAProcess.Stage.RiskChampionHeadInitiateRCSA)

...
{
    public class RiskControlSelfAssessmentEndpointGetAllRCSAForRiskChampionReviewTestApplied
    {
        public static async Task<IResult> HandleAsync(IRepository<RiskControlSelfAssessmentUnit> repository, ClaimsPrincipal user, ICurrentUserService currentUserService)
        {
			try
			{
                ...
                var rcsaList = repository
                    .GetAll().Include(e => e.RiskControlSelfAssessment).Include(e => e.DocumentRSCAProcess)
                    .Where(r => r.DocumentRSCAProcess.RCSAStage == DocumentRSCAProcess.Stage.RiskChampionReviewTestApplied)

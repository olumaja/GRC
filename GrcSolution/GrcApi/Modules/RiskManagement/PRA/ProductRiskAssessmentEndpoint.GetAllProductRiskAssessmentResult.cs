...
namespace GrcApi.Modules.RiskManagement.PRA
{
     ...
     * This endpoint fetch all product risk assessment result
     */
    public class GetAllProductRiskAssessmentResultEndpoint
    {
        /// <summary>
        /// The endpoint fetch all the product risk assessment result
        /// </summary>
        ...
        )
        {
            try
            {
                ...
                var productQuery = productRiskRepo.GetContextByConditon(r => r.EmailAddress.Equals(email.ToString()) && (r.Stage.Equals(PRAStage.RiskChampionReviewPRA) || r.Stage.Equals(PRAStage.Final)))
                    .Select(r => new ProductRiskAssessentDto(
                        r.Id,
                        r.BusinessId,
                        r.DepartmentId,
                        r.UnitId,

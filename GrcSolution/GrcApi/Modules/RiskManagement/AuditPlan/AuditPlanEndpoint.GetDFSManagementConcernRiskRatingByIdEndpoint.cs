...
*/
    public class GetDFSManagementConcernRiskRatingByIdEndpoint
    {
        /// <summary>
        ...
        public static async Task<IResult> HandleAsync(Guid id, IRepository<ManagementConcernRiskRating> managementConcernRating,
            IRepository<ManagementConcernDFS> dfs, IRepository<ManagementConcernBusinessUnitDFSRating> dfsbusiness, IRepository<ManagementConcernSharedServiceDFSRating> dfsshareservice,
            ClaimsPrincipal user, ICurrentUserService currentUserService)
        {
            try
            {
                ...
                if (id != default && getmanagementoncernRating != null)
                {
                    ...
                    var getDFSBusinessRating = dfsbusiness.GetContextByConditon(x => x.ManagementConcernDFSId == getDFSRating.Id).FirstOrDefault();
                    var getDFSSharedserviceRating = dfsshareservice.GetContextByConditon(x => x.ManagementConcernDFSId == getDFSRating.Id).FirstOrDefault();

                    GetDFSManagementConcernRatingById resp = new GetDFSManagementConcernRatingById
                    {
                        BusinessAndBusinessUnit = new BusinessAndBusinessUnitDFSResp
                        {
                            DigitalFinancialService = getDFSBusinessRating.DigitalFinancialService
                        },

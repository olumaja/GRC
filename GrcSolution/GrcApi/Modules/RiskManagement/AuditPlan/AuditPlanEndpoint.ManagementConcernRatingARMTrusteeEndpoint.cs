...
*/
    public class ManagementConcernRatingARMTrusteeEndpoint
    {
        /// <summary>
        ...
        /// <returns></returns>        
        public static async Task<IResult> HandleAsync([FromBody] ManagementConcernARMTrusteeRiskRatingRequest mgtConcernRatingReq, IRepository<ManagementConcernRiskRating> managementConcernRating, IRepository<ManagementConcernARMTrustee> mgtConcernRating,
            IRepository<ManagementConcernBusinessUnitARMTrusteeRating> businessRating, IRepository<ManagementConcernSharedServiceARTrusteeRating> shareserviceRating,
            IRepository<BusinessRiskRating> businessRiskRating, ClaimsPrincipal user, IConfiguration config, IEmailService EmailService, ICurrentUserService currentUserService)
        {
            try
            {
                ...
                if (unit != "ARM Trustees") { return TypedResults.Forbid(); }
                
                var getBusinessRiskRating = businessRiskRating.GetContextByConditon(x => x.Id == mgtConcernRatingReq.ARMTrustee.BusinessRiskRatingId).FirstOrDefault();
                if (getBusinessRiskRating == null)
                {
                    return TypedResults.Ok($"The Business Risk Rating with id {mgtConcernRatingReq.ARMTrustee.BusinessRiskRatingId} does not exist");
                }
                ...
                if (getId != null)
                {
                    ...
                    var saveMgtConcern = ManagementConcernARMTrustee.Create(getId.Id, requesterName, mgtConcernRatingReq.ARMTrustee.Comment);
                    await mgtConcernRating.AddAsync(saveMgtConcern);
                    //log business and business unit
                    var saverating = ManagementConcernBusinessUnitARMTrusteeRating.Create(mgtConcernRatingReq.ARMTrustee, saveMgtConcern.Id);
                    await businessRating.AddAsync(saverating);
                    //log shared service
                    var saveSharedServicerating = ManagementConcernSharedServiceARTrusteeRating.Create(mgtConcernRatingReq.ARMTrustee, saveMgtConcern.Id);
                  ...
                  //  await businessRiskRating.SaveChangesAsync();
                    ...
                    var linkToGRCTool = string.Format(config["EmailConfiguration:InternalAuditSupervisorApprovalLink"], mgtConcernRatingReq.ARMTrustee.BusinessRiskRatingId);
                    string body = $"Dear Team, <br><br> {requesterName} has completed the risk assessment for ARMTrustee.<br><br>Please click the following link <a href={linkToGRCTool}>GRC link</a> to view the assessment.";
                ...
                {
                    //log req
                    var saveMgtConcernRating = ManagementConcernRiskRating.CreateManagementConcernRiskRating(mgtConcernRatingReq.ARMTrustee.BusinessRateRequesterName, requesterName, mgtConcernRatingReq.ARMTrustee.BusinessRiskRatingId);
                    await managementConcernRating.AddAsync(saveMgtConcernRating);
                    //log requester email and comment
                    var saveMgtConcern2 = ManagementConcernARMTrustee.Create(saveMgtConcernRating.Id, requesterName, mgtConcernRatingReq.ARMTrustee.Comment);
                    await mgtConcernRating.AddAsync(saveMgtConcern2);
                    //log business and business unit
                    var saverating2 = ManagementConcernBusinessUnitARMTrusteeRating.Create(mgtConcernRatingReq.ARMTrustee, saveMgtConcern2.Id);
                    await businessRating.AddAsync(saverating2);
                    //log shared service
                    var saveSharedServicerating2 = ManagementConcernSharedServiceARTrusteeRating.Create(mgtConcernRatingReq.ARMTrustee, saveMgtConcern2.Id);

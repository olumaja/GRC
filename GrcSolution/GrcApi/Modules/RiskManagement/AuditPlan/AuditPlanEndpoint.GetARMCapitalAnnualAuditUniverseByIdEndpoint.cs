...
namespace Arm.GrcApi.Modules.RiskManagement.AuditPlan
{
  ...
  */
    public class GetARMCapitalAnnualAuditUniverseByIdEndpoint
    {
        /// <summary>
        /// Endpoint to Get ARMCapital Annual Audit Universe By Id
        /// </summary>
        ...
        public static async Task<IResult> HandleAsync(Guid anualAuditUniverseId, IRepository<AnualAuditUniverseRiskRating> dfsannualaudit,
            IRepository<ARMCapitalAuditUniverse> dfsreq, IRepository<AuditUniverseARMCapitalRating> dfsRate,
            ClaimsPrincipal user, ICurrentUserService currentUserService)
        {
            try
            {
                ...
                var getdfsRate = dfsRate.GetContextByConditon(x => x.ARMCapitalAuditUniverseId == getAuditRating.Id).FirstOrDefault();

                GetARMCapitalAuditUniverseSummary resp = new GetARMCapitalAuditUniverseSummary
                {
                    AnualAuditUniverseId = anualAuditUniverseId,
                    ARMCapital = new AuditUniverseSummary
                    {

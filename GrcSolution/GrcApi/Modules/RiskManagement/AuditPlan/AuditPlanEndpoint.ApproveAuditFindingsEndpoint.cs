...
{
    public class ApproveAuditFindingsEndpoint
    {
        public static async Task<IResult> HandleAsync(
            ApproveAuditFindingsDto payload, IRepository<CommenceEngagementAuditexecution> commenceRepo, IRepository<AuditProgramAuditExecution> programRepo,
        ...
        )
        {
            try
            {
                ...
                getprogramRepo.UpdateFindingStatus(payload.Team);
                commenceEngagement.ApproveFindingstatusStatus(payload.Team);
                getfindingsRepo.ApproveFindingStatus(payload.Team);
                ...
                string body = $"Dear {getAuditAnnouncementMemo.RequesterName},<br> {requesterName} has approved the audit finding for {getAuditAnnouncementMemo.Unit} {getAuditAnnouncementMemo.Recommendation}, for your review and approval.<br><br>Please click the following link <a href={linkToGRCTool}>GRC link</a> to view the assessment.<br><br>Thank you";
                ...
                #endregion
                return TypedResults.Ok("Audit findings approved");
            }
            catch (Exception ex)
            {
    ...
    public record ApproveAuditFindingsDto(Guid AuditProgramId, string Team);

    public class ApproveAuditFindingsDtoValidation : AbstractValidator<ApproveAuditFindingsDto>
    {
        public ApproveAuditFindingsDtoValidation()
        {

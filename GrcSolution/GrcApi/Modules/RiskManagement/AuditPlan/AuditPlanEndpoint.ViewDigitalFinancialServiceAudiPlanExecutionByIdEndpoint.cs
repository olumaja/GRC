...
namespace Arm.GrcApi.Modules.RiskManagement.AuditPlan
{
   ...
   */
    public class ViewDigitalFinancialServiceAudiPlanExecutionByIdEndpoint
    {
        /// <summary>
        ...
        public static async Task<IResult> HandleAsync(Guid anualAuditUniverseId, IRepository<AnualAuditUniverseRiskRating> dfsAnnualaudit,
            IRepository<DigitalFinancialServiceAuditUniverse> dfsReq, IRepository<CommenceEngagementAuditexecution> commenceEng,
            IRepository<AuditUniverseDigitalFinancialServiceRating> dfsRating, IRepository<AuditProgramAuditExecution> auditProgram, ClaimsPrincipal user, ICurrentUserService currentUserService)
        {
            try
            {
                ...
                var getCommenceEngDFS = commenceEng.GetContextByConditon(x => x.AnualAuditUniverseRiskRatingId == getAuditRating.AnualAuditUniverseRiskRatingId && x.Team == "Digital Financial Service").FirstOrDefault();
                ...
                if (getauditProgram.Status == BusinessRiskRatingStatus.Approved && getauditProgram.Team == "Digital Financial Service")
                {
                    engstatus = BusinessRiskRatingStatus.Approved;
                }
                if (getauditProgram.Status == BusinessRiskRatingStatus.Completed && getauditProgram.Team == "Digital Financial Service")
                {
                    engstatus = BusinessRiskRatingStatus.Completed;
                }
                var getdfsRating = dfsRating.GetContextByConditon(x => x.DigitalFinancialServiceAuditUniverseId == getAuditRating.Id).FirstOrDefault();
                string month = null;

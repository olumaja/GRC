 ...
 * This endpoint allow Management Concern to perform risk rating on the ARMCapital business entities
 */
    public class ManagementConcernRatingARMCapitalEndpoint
    {
        /// <summary>
        /// Management Concern to perform risk rating on the ARMCapital business entities
        ...
        /// <returns></returns>
        public static async Task<IResult> HandleAsync([FromBody] ManagementConcernARMCapitalRequest mgtConcernRatingReq, IRepository<ManagementConcernRiskRating> managementConcernRating, IRepository<ManagementConcernARMCapital> mgtConcernRating,
                     IRepository<ManagementConcernBusinessUnitARMCapitalRating> businessRating, IRepository<ManagementConcernSharedServiceARMCapitalRating> shareserviceRating,
                     IRepository<BusinessRiskRating> businessRiskRating, ClaimsPrincipal user, IConfiguration config, IEmailService EmailService, ICurrentUserService currentUserService)
        {
            try
            {
                ...
                if (unit != "Investment Banking") { return TypedResults.Forbid(); }

                var getBusinessRiskRating = businessRiskRating.GetContextByConditon(x => x.Id == mgtConcernRatingReq.ARMCapital.BusinessRiskRatingId).FirstOrDefault();
                if (getBusinessRiskRating == null)
                {
                    return TypedResults.Ok($"The Business Risk Rating with id {mgtConcernRatingReq.ARMCapital.BusinessRiskRatingId} does not exist");
                }
                ...
                if (getId != null)
                {
                    ...
                    var saveMgtConcern = ManagementConcernARMCapital.Create(getId.Id, requesterName, mgtConcernRatingReq.ARMCapital.Comment);
                    await mgtConcernRating.AddAsync(saveMgtConcern);
                    //log business and business unit
                    var saverating = ManagementConcernBusinessUnitARMCapitalRating.Create(mgtConcernRatingReq.ARMCapital, saveMgtConcern.Id);
                    await businessRating.AddAsync(saverating);
                    //log shared service
                    var saveSharedServicerating = ManagementConcernSharedServiceARMCapitalRating.Create(mgtConcernRatingReq.ARMCapital, saveMgtConcern.Id);
                   ...
                   // await businessRiskRating.SaveChangesAsync();
                    ...
                    var linkToGRCTool = string.Format(config["EmailConfiguration:InternalAuditSupervisorApprovalLink"], mgtConcernRatingReq.ARMCapital.BusinessRiskRatingId);
                    string body = $"Dear Team,<br><br> {requesterName} has completed the risk assessment for ARMCapital.<br><br>Please click the following link <a href={linkToGRCTool}>GRC link</a> to view the assessment.";
                ...
                {
                    //log req
                    var saveMgtConcernRating = ManagementConcernRiskRating.CreateManagementConcernRiskRating(mgtConcernRatingReq.ARMCapital.BusinessRateRequesterName, requesterName, mgtConcernRatingReq.ARMCapital.BusinessRiskRatingId);
                    await managementConcernRating.AddAsync(saveMgtConcernRating);
                    //log requester email and comment
                    var saveMgtConcern2 = ManagementConcernARMCapital.Create(saveMgtConcernRating.Id, requesterName, mgtConcernRatingReq.ARMCapital.Comment);
                    await mgtConcernRating.AddAsync(saveMgtConcern2);
                    //log business and business unit
                    var saverating2 = ManagementConcernBusinessUnitARMCapitalRating.Create(mgtConcernRatingReq.ARMCapital, saveMgtConcern2.Id);
                    await businessRating.AddAsync(saverating2);
                    //log shared service
                    var saveSharedServicerating2 = ManagementConcernSharedServiceARMCapitalRating.Create(mgtConcernRatingReq.ARMCapital, saveMgtConcern2.Id);
                   ...
                   // await businessRiskRating.SaveChangesAsync();
                    ...
                    string emailTo = config["EmailConfiguration:InternalAuditSupervisor"];
                    string toCC = config["EmailConfiguration:InternalAudit"];
                    var linkToGRCTool2 = string.Format(config["EmailConfiguration:InternalAuditSupervisorApprovalLink"], mgtConcernRatingReq.ARMCapital.BusinessRiskRatingId);
                    string body = $"Dear Team,<br><br> {requesterName} has completed the risk assessment for ARMCapital.<br><br>Please click the following link <a href={linkToGRCTool2}>GRC link</a> to view the assessment.";

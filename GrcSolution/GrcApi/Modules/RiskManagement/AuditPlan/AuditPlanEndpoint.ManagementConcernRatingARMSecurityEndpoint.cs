...
*/
    public class ManagementConcernRatingARMSecurityEndpoint
    {
        ...
        /// <returns></returns>
        public static async Task<IResult> HandleAsync([FromBody] ManagementConcernARMSecurityRiskRatingRequest mgtConcernRatingReq, IRepository<ManagementConcernRiskRating> managementConcernRating, IRepository<ManagementConcernARMSecurity> mgtConcernRating,
            IRepository<ManagementConcernBusinessUnitARMSecurityRating> businessRating, IRepository<ManagementConcernSharedServiceARMSecurityRating> shareserviceRating,
...

        {
            try
            {
                ...
                if (unit != "ARM Securities") { return TypedResults.Forbid(); }

                var getBusinessRiskRating = businessRiskRating.GetContextByConditon(x => x.Id == mgtConcernRatingReq.ARMSecurity.BusinessRiskRatingId).FirstOrDefault();
                if (getBusinessRiskRating == null)
                {
                    return TypedResults.Ok($"The Business Risk Rating with id {mgtConcernRatingReq.ARMSecurity.BusinessRiskRatingId} does not exist");
                }
                ...
                if (getId != null)
                {
                    ...
                    var saveMgtConcern = ManagementConcernARMSecurity.Create(getId.Id, requesterName, mgtConcernRatingReq.ARMSecurity.Comment);
                    await mgtConcernRating.AddAsync(saveMgtConcern);
                    //log business and business unit
                    var saverating = ManagementConcernBusinessUnitARMSecurityRating.Create(mgtConcernRatingReq.ARMSecurity, saveMgtConcern.Id);
                    await businessRating.AddAsync(saverating);
                    //log shared service
                    var saveSharedServicerating = ManagementConcernSharedServiceARMSecurityRating.Create(mgtConcernRatingReq.ARMSecurity, saveMgtConcern.Id);
                    ...
                    var linkToGRCTool = string.Format(config["EmailConfiguration:InternalAuditSupervisorApprovalLink"], mgtConcernRatingReq.ARMSecurity.BusinessRiskRatingId);
                    string body = $"Dear Team,<br><br> {requesterName} has completed the risk assessment for ARMSecurities.<br><br>Please click the following link <a href={linkToGRCTool}>GRC link</a> to view the assessment.";
                ...
                {
                    //log req
                    var saveMgtConcernRating = ManagementConcernRiskRating.CreateManagementConcernRiskRating(mgtConcernRatingReq.ARMSecurity.BusinessRateRequesterName, requesterName, mgtConcernRatingReq.ARMSecurity.BusinessRiskRatingId);
                    await managementConcernRating.AddAsync(saveMgtConcernRating);
                    //log requester email and comment
                    var saveMgtConcern2 = ManagementConcernARMSecurity.Create(saveMgtConcernRating.Id, requesterName, mgtConcernRatingReq.ARMSecurity.Comment);
                    await mgtConcernRating.AddAsync(saveMgtConcern2);
                    //log business and business unit
                    var saverating2 = ManagementConcernBusinessUnitARMSecurityRating.Create(mgtConcernRatingReq.ARMSecurity, saveMgtConcern2.Id);
                    await businessRating.AddAsync(saverating2);
                    //log shared service
                    var saveSharedServicerating2 = ManagementConcernSharedServiceARMSecurityRating.Create(mgtConcernRatingReq.ARMSecurity, saveMgtConcern2.Id);

 ...
 */
    public class ApproveDFSBusinessRiskRatingEndpoint
    {

        /// <summary>
        /// Endpoint to approve DFS business risk rating
        /// </summary>
        ...
        /// <returns></returns>
        public static async Task<IResult> HandleAsync(Guid businessriskratingId, [FromBody] DigitalFinancialServicesBusinessRiskRatingReq request,
            IRepository<BusinessRiskRating> busnessRiskRating, IRepository<DigitalFinancialServiceBusinessRiskRating> armDFS, IRepository<StrategyDigitalFinancialService> strategy,
            IRepository<StrategyDigitalFinancialServiceRating> strategyDFS,
            ...
            IRepository<AnualAuditUniverseRiskRating> annualaudit, IRepository<DigitalFinancialServiceAuditUniverse> armtamauditUniverse,
            IRepository<AuditUniverseDigitalFinancialServiceRating> logTamaudit, ICurrentUserService currentUserService,
            IRepository<EMCConcernRiskRating> emcRating, IRepository<DigitalFinancialServiceEMCRating> dfsEMCRating, IRepository<ManagementConcernRiskRating> managementConcernRating,
            IRepository<ManagementConcernDFS> mcRating, IRepository<ManagementConcernBusinessUnitDFSRating> busdfsMCRating,  IRepository<ManagementConcernSharedServiceDFSRating> sharedfsMCRating, 
            ClaimsPrincipal user)
        {
            try
            {
                ...
                var getstrategyDFS = strategyDFS.GetContextByConditon(x => x.StrategyDigitalFinancialServiceId == getstrategylog.Id).FirstOrDefault();
                ...
                var getoperDFS = operDFS.GetContextByConditon(x => x.OperationDigitalFinancialServiceId == getoperation.Id).FirstOrDefault();
                ...
                var getfinanceDFS = financeDFS.GetContextByConditon(x => x.FinancialDigitalFinancialServiceId == getfinance.Id).FirstOrDefault();
                ...
                var getcomplianceDFS = complianceDFS.GetContextByConditon(x => x.ComplianceDigitalFinancialServiceId == getcompliance.Id).FirstOrDefault();
                ...
                //var getMgtRatingbusiness = busdfsMCRating.GetContextByConditon(x => x.ManagementConcernDFSId == getMCConcernRating.Id).FirstOrDefault();
                //var getMgtSharedService = sharedfsMCRating.GetContextByConditon(x => x.ManagementConcernDFSId == getMCConcernRating.Id).FirstOrDefault();
                ...
                {
                    var getEmcConcernDFS = dfsEMCRating.GetContextByConditon(x => x.EMCConcernRiskRatingId == item1.Id).Select(x => x.DigitalFinancialService).ToList();
                ...
                foreach (var item2 in getMCRating)
                {
                    ...
                    {
                        var getbusdfsMCRating = busdfsMCRating.GetContextByConditon(x => getMCConcernDFS.Select(y => y.Id).Contains(x.ManagementConcernDFSId)).Select(x => x.DigitalFinancialService).ToList();
                ...
                var RiskScoreTimesinceLastauditDFS = (gettimeSinceLastARMTAMAudit.DigitalFinancialService / (double)maximumPossibleTimeSinceLastAuditBusinessMgtRating) * timeSinceLastAuditWeight;
                ...
                if (getannualauditId == null)
                {
                    ...
                    var dfs = DigitalFinancialServiceAuditUniverse.Create(getTAMRating.RequesterName, logRequestId.Id);
                    await armtamauditUniverse.AddAsync(dfs);

                    var logdfsReq = AuditUniverseDigitalFinancialServiceRating.Create(dfs.Id, oldRiskScoredfs.ToString("0.000"), businessManagerConcerndfs, directorManagerConcernDFS,
                ...
                if (getannualauditId != null)
                {
                    ...
                    if (getTAM == null)
                    {
                        var armtam2 = DigitalFinancialServiceAuditUniverse.Create(getTAMRating.RequesterName, getannualauditId.Id);
                        await armtamauditUniverse.AddAsync(armtam2);

                        var logdfsReq2 = AuditUniverseDigitalFinancialServiceRating.Create(armtam2.Id, oldRiskScoredfs.ToString("0.000"), businessManagerConcerndfs, directorManagerConcernDFS,
                                        riskScoredfs.ToString("0.000"), riskRatingdfs, recommentationdfs);

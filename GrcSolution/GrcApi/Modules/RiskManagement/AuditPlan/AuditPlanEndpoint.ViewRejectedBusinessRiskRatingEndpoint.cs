...
namespace Arm.GrcApi.Modules.RiskManagement.AuditPlan
{
  ...
  */
    public class ViewRejectedBusinessRiskRatingEndpoint
    {

        /// <summary>
        /// Get all rejected business risk rating 
        /// </summary>
        ...
        public static async Task<IResult> HandleAsync(IRepository<BusinessRiskRating> repo, IRepository<EMCConcernRiskRating> getEmcConcern,
            IRepository<ManagementConcernRiskRating> managementConcernRating, ICurrentUserService currentUserService,
            ClaimsPrincipal user)
        {
            try
            {
                ...
                var getBusinessRiskRating = repo.GetContextByConditon(c => c.IsEMCRated == true && c.IsManagementRated == true && c.Status == BusinessRiskRatingStatus.Rejected).ToList();
                if (getBusinessRiskRating == null)
                {
                    return TypedResults.NotFound();
                }
                var emcbusinesse = await getEmcConcern.GetAllAsync();
                var mcbusinesse = await managementConcernRating.GetAllAsync();
                if (getBusinessRiskRating != null)
                {
                    foreach (var item in getBusinessRiskRating)
                    {
                        var geaEMCRating = emcbusinesse.Find(x => x.BusinessRiskRatingId.Equals(item.Id));

...
namespace GrcApi.Modules.RiskManagement.AuditPlan
{
   ...
   */
    public class ApproveAuditPlanningMemoEndpoint
    {
        ...
        public static async Task<IResult> HandleAsync(
            ApproveAuditPlanningMemoRequest payload, IRepository<AuditPlanningMemoAuditExecution> planingMemo, IRepository<AuditAnnouncementMemoAuditExecution> auditmemo,
            IRepository<CommenceEngagementAuditexecution> commenceEng, IRepository<AuditPlanningMemoPlanningTimeline> planningTime, IEmailService EmailService, IConfiguration config, ClaimsPrincipal user, ICurrentUserService currentUserService
        )
        {
            try
            {
                ...
                var getAuditPlanningMemo = planingMemo.GetContextByConditon(x => x.CommenceEngagementAuditexecutionId == getCommenceEng.Id && x.Team == payload.Team).FirstOrDefault();
                var getplanningTime = planningTime.GetContextByConditon(x => x.AuditPlanningMemoAuditExecutionId == getAuditPlanningMemo.Id).FirstOrDefault();
                ...
                if (getAuditPlanningMemo != null)
                {
                    getAuditPlanningMemo.ApproveAuditPlanningMemoStatus();
                    auditmemo.SaveChanges();
                    #region Send email to the supervisor 
                   
                    string subject = $"Audit Planing Memo - {getAuditAnnouncementMemo.Unit}";
                    ...
                    string bodyOfficer = $"Dear {getAuditAnnouncementMemo.RequesterName},<br> {requesterName} has approved the Audit Planing Memo for {getAuditAnnouncementMemo.Unit} {getAuditAnnouncementMemo.Recommendation}, for your review and approval.<br><br>Please click the following link <a href={linkToGRCTool}>GRC link</a> to view the assessment.<br><br>Thank you";
    ...
    public record ApproveAuditPlanningMemoRequest(Guid CommenceEngagementId, string Team);

    public class ApproveAuditPlanningMemoRequestValidation : AbstractValidator<ApproveAuditPlanningMemoRequest>
    {
        public ApproveAuditPlanningMemoRequestValidation()
        {

...
namespace Arm.GrcApi.Modules.OperationControl
{
     ...
     */
    public class GetOperationExceptionEndpoint
    {
        /// <summary>
        /// Get all operation exceptions
        /// </summary>
        ...
        public static async Task<IResult> HandleAsync(
            int pageSize, int pageNumber, string? resolutionStatus, string? exceptionCategory, string? businessUnit, DateTime? startDate, DateTime? endDate, string? loggedBy, 
            string? search, IRepository<OperationControl> repo, ICurrentUserService currentUserService, IHttpContextAccessor httpContext
        )
        {
            ...
            try
            {
                ...
                var myExceptions = repo.GetAll();
                                   //.OrderByDescending(r => r.CreatedOnUtc);                             

                if (loggedBy != null)
                {
                    var checkIfOfficerExist = repo.GetContextByConditon(x => x.CreatedBy == loggedBy).FirstOrDefault();
                    if (checkIfOfficerExist == null)
                    {
                        return TypedResults.BadRequest("Operation officer does not exist");
                    }

                    myExceptions = myExceptions.Where(d => d.CreatedBy == loggedBy);
                                  //.OrderByDescending(r => r.CreatedOnUtc);
                }

                if (businessUnit != null)
                {
                    var checkIfbusinessUnitExist = repo.GetContextByConditon(x => x.Unit == businessUnit).FirstOrDefault();
                    if (checkIfbusinessUnitExist == null)
                    {
                        return TypedResults.BadRequest("Business unit does not exist");
                    }
                    myExceptions = myExceptions.Where(d => d.Unit == businessUnit);
                                  //.OrderByDescending(r => r.CreatedOnUtc);
                }
                ...
                if (!string.IsNullOrWhiteSpace(resolutionStatus))
                {
                    ...
                    myExceptions = myExceptions.Where(d => d.ResolutionStatus == statusRecommended[getStatus]);
                                //.OrderByDescending(r => r.CreatedOnUtc);
                }
                ...
                if(!string.IsNullOrWhiteSpace(search)) {
                    myExceptions = myExceptions.Where(d => d.ExceptionDescription.Contains(search) || d.ActionPoint.Contains(search) || d.ActionOwner.Contains(search) ||
                        d.CreatedBy.Contains(search) || d.OperationActivity.Contains(search) || d.TransactionCallOverType.Contains(search) || d.Unit.Contains(search)
                    );
                }

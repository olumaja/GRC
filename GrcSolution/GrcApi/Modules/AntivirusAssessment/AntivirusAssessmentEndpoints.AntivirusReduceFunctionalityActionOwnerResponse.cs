...
{
    public class AntivirusReduceFunctionalityActionOwnerResponseEndpoint
    {
        public static async Task<IResult> HandleAsync(
            HttpRequest payload, IRepository<AntivirusAssessmentModel> antiVirusRepo, IRepository<ReducedFunctionalityMode> reduceRepo,
        ...
        )
        {
            ...
            while (true)
            {
                ...
                var reduceFunctionalityIdKey = $"ReduceFunctionalityId{count}";
                var statusKey = $"Status{count}";
                var actionOwnerCommentKey = $"ActionOwnerComment{count}";
                var evidenceFileKey = $"EvidenceFile{count}";

                if (!forms.ContainsKey(antiVirusModelIdKey) || !forms.ContainsKey(reduceFunctionalityIdKey) || !forms.ContainsKey(statusKey))
                    break;

                Guid antiVirusModelId;
                Guid reduceFunctionalityId;
                ...
                if (!Guid.TryParse(forms[reduceFunctionalityIdKey], out reduceFunctionalityId))
                    return TypedResults.BadRequest($"The reduce functionality id {forms[reduceFunctionalityIdKey]} is not valid");
                ...
                evidences.Add(new AntivirusAssessmentDTO(
                    ...
                    InactiveSensorId: reduceFunctionalityId,
            ...
            antivirus.ReduceFunctionalityStatusAfterActionOwnerResponse();
            antivirus.SetModifiedBy(currentUserService.CurrentUserFullName);
            antivirus.SetModifiedOnUtc(DateTime.Now);

            var reducedFunctionality = antivirus.ReducedFunctionalityMode;
            ...
            {
                var index = reducedFunctionality.FindIndex(v => v.Id == evidence.InactiveSensorId);

                if (index >= 0)
                {
                    reducedFunctionality[index].UpdateAfterActionOwnerResponse(evidence.Status, evidence.ActionOwnerComment);
                    if (evidence.Attachment != null)
                        attachedFiles.Add(DocumentExtensions.ConvertFormFileToDocument(evidence.Attachment, ModuleType.InfoSecAntivirusReduceFunctionality, evidence.InactiveSensorId));
                }
